...
public enum Derivation {
    public static func members(of declaration: some DeclGroupSyntax) throws -> [DeclSyntax] {
        guard let structure = declaration.as(StructDeclSyntax.self) else { throw AlgebraDiagnostic("@Representable requires a fixed homogeneous product") }
        ...
        guard shape.properties.fields.allSatisfy({ $0.type.trimmedDescription == parameter }) else {
            throw AlgebraDiagnostic("@Representable requires every stored coordinate to be the parameter itself; metadata and variable shapes are not representable by this derivation")
        }
    ...
    public static func index(of declaration: some DeclSyntaxProtocol) throws -> [DeclSyntax] {
        guard let structure = declaration.as(StructDeclSyntax.self) else { throw AlgebraDiagnostic("@Representable requires a struct") }
        let shape = try GenericProduct(structure, arity: 1)
        guard shape.properties.fields.allSatisfy({ $0.type.trimmedDescription == shape.parameters[0] }) else {
            throw AlgebraDiagnostic("@Representable requires a fixed homogeneous product")
        }
